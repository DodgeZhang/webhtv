...
=======
=======
Android Studio 仅是可选 IDE，不是项目依赖。Gradle Wrapper 和仓库脚本不会读取 Android Studio 的安装目录或内置 JBR；命令行构建只使用 `JAVA_HOME`/`PATH` 中的 JDK，以及 `ANDROID_HOME`、`ANDROID_SDK_ROOT` 或 `local.properties` 指定的 Android SDK。
...
命令行构建需要根目录有 `local.properties`。它是普通文本配置，在 clone 后进入仓库根目录直接生成，不需要 Android Studio：
...
<<<<<<< HEAD
=======
`platforms;android-37.0` 是当前 `sdkmanager` 返回的实际包名；Gradle 中仍写 `compileSdk=37`。如果 `cmdline-tools/latest/bin/sdkmanager` 不存在，请安装官方 Android SDK Command-line Tools，并将其按 `cmdline-tools/latest/` 目录结构放入 SDK；Android Studio 的 SDK Manager 仅是可选的图形化安装方式。
...
准备工具链：
=======
| FFmpeg 配置基线 | `96598d75d381a4e8c3137795a7b490253a26ec80` |
| FFmpeg | `ff4.0--ijk0.8.8--20210426--001` (`e88b5afbf9ca543a098ede6dd6765ef0e629836d`) |
| OpenSSL | `openssl-3.2` (`14ddbcee237cb99b3921c352852b4d4fadbb8e6c`) |
...
<<<<<<< HEAD
=======
| `.../cmdline-tools/latest/bin/sdkmanager: No such file or directory` | 安装官方 Android SDK Command-line Tools，确认其位于 `$ANDROID_HOME/cmdline-tools/latest/`，并检查 `ANDROID_HOME`；不要求安装 Android Studio |
>>>>>>> upstream/beta
