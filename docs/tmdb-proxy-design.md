...
- 验收：TMDB 配置只显示一个访问线路输入框；默认显示官方直连地址；可选择内置线路或输入自定义 endpoint；API 与默认图片请求随线路切换；旧配置字段仍兼容；聚焦单测和 Debug 资源/Java 编译通过。
...
本次将这三个入口合并为一个 **TMDB 访问线路** 输入框：默认显示原来的官方直连地址，点击下拉可选择内置代理，也可直接输入自定义 endpoint。旧配置中的 `apiBase`、`imageBase` 仍保留在数据模型中用于兼容，但不再在普通配置界面重复展示。
...
- 访问线路输入官方地址时使用直连；输入其他自定义 endpoint 或选择内置线路时，保存为代理线路并让 API/默认图片共同跟随；
- 未重新选择代理时，旧配置中的显式图片域名仍被保留；选择代理线路后省略旧图片覆盖，让图片随代理 endpoint；
- 未选代理时，保持现有官方直连行为。
...
| **本项目选择：单一 endpoint 线路输入框** | 与 alist-tvbox 的实际实现一致；内置 Worker 轮询池、itv666、NAStool，并支持自定义单地址/地址池；默认图片跟随代理；旧 API/图片字段只作为兼容数据保留。 |
...
- UI 只显示一个 TMDB 访问线路下拉输入控件；独立标签与控件 hint 不重复，避免 Android 9 上的文字重叠。

## 验收与回滚

验收至少包括：

1. XML 中 `ScrollView` 只有一个直接子节点，TMDB 访问线路只有一个下拉输入控件且不再使用会重叠的双 hint 结构；
...
根据使用反馈，已将 API 域名、图片域名和代理线路的普通 UI 入口收敛为一个 `TMDB 访问线路` 输入框：

- 默认显示 `https://api.tmdb.org`；点击右侧下拉箭头显示官方直连、Worker 轮询池、itv666、NAStool；也可直接编辑自定义 endpoint；
- 官方 API 地址保存为直连 `apiBase`；其他地址保存为 `proxyBase`，自动覆盖 API 和默认图片路径；
- 旧配置的显式 `imageBase` 在直连保存时保留，切换代理线路时清除覆盖以让图片跟随代理；
- 原有内部配置字段和运行时回退逻辑不删除，避免升级后丢失订阅/迁移配置。

## 实施与验证记录（2026-09-22）

- 已将 TMDB 线路控件改为独立标签 + 下拉选择框；不再同时展示 API 域名、图片域名和代理输入框，且不使用外层 label hint 与子控件 hint，解决截图中的文字重叠。
...
- 布局结构检查通过：`ScrollView` 只有一个直接子节点，`apiHostInput` 作为唯一 TMDB 线路输入控件出现一次，`imageHostInput`/`proxyHostInput` 不再存在于该布局。
...
- 已使用 `scripts/build_arm64_debug_install.sh --flavor mobile --abi arm64-v8a --serial 192.168.50.3:5557` 覆盖安装 Debug 包并进入 TMDB 配置弹窗检查；运行时 UI dump 确认默认显示 `https://api.tmdb.org`，仅存在一个线路输入控件，并实际打开下拉查看内置线路；未卸载已有包，未生成正式包。
