...
| --- | --- | --- | --- | --- |
| `E-SP9` | Exo 性能/播放行为 | 硬解模式在平台无该 MIME 硬件候选时使用 FFmpeg 软解兜底，避免音频正常但视频轨静默不选 | **设备验证通过，待 guard 收口**：定向测试与 arm64 测试包构建通过；5561 的 H.265 直播从 shutter 黑屏恢复为可见画面，实际软解解码器、1280x720 尺寸和首帧均已记录 | [E-SP9-exo-unsupported-video-soft-fallback.md](E-SP9-exo-unsupported-video-soft-fallback.md) |
