...
| `C16` | common / 详情数据协议 | 参考 OmniBox，让 T3/T4 详情直接携带 TMDB 数据，APP 优先采用并仅补齐缺省部分；第 19 节评估详情响应最外层 `tmdb_api_key`，用户未配置时放入进程内临时全局凭据池，在 App 进程存活期间跨页面/跨源/切换订阅接口使用且不持久化 | **客户端第 1-18 节已实现并验证；第 19 节设计评审已完成，待用户批准实施**：阶段 A-F 提交为 `19128b7d18107b5c614078ebdadc863faa2476b5`、`1c4213db0baf24e9da9495edfdbaa9dcab43f527`、`6a994fa1acd85cd307bf111bc077f6e5a1385da0`、`b2343d1d1de476226b52bef11b80248e1d5033d9`、`6e2cde565ae34c1a0da392c7ca93f34cb97aaa99`、`d52fdfc5b2027872779026bad278c1a5c8e9f5d8`；V1923A/Android 9 5559 上 Mobile/Leanback 无 Key 设备矩阵各 3/3。第 19 节结论为有条件实施，必须先封闭日志、缓存、Parcel、Intent、自定义 API Base 和旧客户端泄漏路径。T4 生产服务端仍待独立接入 | [C16-tmdb-source-detail-contract.md](C16-tmdb-source-detail-contract.md) |
